...
class TicTacToeApp:
    ...
    def make_move(self, row, col):
        ...
        if cell.cget("text") == "":
            ...
            if self.check_winner_by_color(self.ai_color):
                messagebox.showinfo("Game Over", "IA wins!")
                self.settings_page()
                return
    ...
    def minimax(self, is_ai_turn, depth, alpha, beta):
        ...
        elif self.check_draw() or depth == 0:
            return 0  # Égalité ou profondeur atteinte
    ...
    def find_best_move(self):
        ...
        for (r, c) in empty_cells:
            self.grid[r][c].configure(fg_color=self.ai_color)
            score = self.minimax(False, depth=self.ai_depth, alpha=float("-inf"), beta=float("inf"))
            self.grid[r][c].configure(fg_color="gray")
            if score > best_score:
                best_score = score
                best_move = (r, c)
    ...
    def check_winner_by_color(self, color):
        ...
        if all(self.grid[i][rows - 1 - i].cget("fg_color") == color for i in range(rows)):
            return True
